...
public class DynamicRoutingTests
{
    ...
    {
        await using var server = await TestServer.StartAsync(app => app.MapGet("/first", ctx => ctx.Response.WriteAsync("first")));

        Assert.Equal(HttpStatusCode.NotFound, (await server.Client.GetAsync("/second", Token)).StatusCode);

        server.Server.MapGet("/second", ctx => ctx.Response.WriteAsync("second"));
    ...
    public async Task Removes_a_route_while_running()
    {
        ...
        {
            app.MapGet("/keep", ctx => ctx.Response.WriteAsync("keep"));
            app.MapGet("/drop", ctx => ctx.Response.WriteAsync("drop"));
        });
    ...
    {
        await using var server = await TestServer.StartAsync(app => app.MapGet("/stay", ctx => ctx.Response.WriteAsync("stay")));
    ...
    {
        await using var server = await TestServer.StartAsync(app => app.MapGet("/x", ctx => ctx.Response.WriteAsync("x")));
    ...
    {
        await using var server = await TestServer.StartAsync(app => app.MapGet("/toggle", ctx => ctx.Response.WriteAsync("on")));
        ...
        server.Server.MapGet("/toggle", ctx => ctx.Response.WriteAsync("back on"));
    ...
    public async Task Clears_every_route_and_falls_through_to_the_terminal_handler()
    {
        await using var server = await TestServer.StartAsync(app =>
        {
            app.MapGet("/a", ctx => ctx.Response.WriteAsync("a"));
            app.MapGet("/b", ctx => ctx.Response.WriteAsync("b"));
    ...
    public async Task Removes_a_group_of_routes_by_predicate()
    {
        await using var server = await TestServer.StartAsync(app =>
        {
            app.MapGet("/admin/one", ctx => ctx.Response.WriteAsync("1"));
            app.MapGet("/admin/two", ctx => ctx.Response.WriteAsync("2"));
            app.MapGet("/public", ctx => ctx.Response.WriteAsync("p"));
        });
    ...
    {
        await using var server = await TestServer.StartAsync(app => app.MapGet("/stable", ctx => ctx.Response.WriteAsync("stable")));
        ...
        var churn = Task.Run(
            () =>
            {
                ...
                {
                    server.Server.MapGet("/churn", ctx => ctx.Response.WriteAsync("churn"));
    ...
    public async Task Routes_survive_a_stop_and_start()
    {
        ...
        server.Server.MapGet("/added", ctx => ctx.Response.WriteAsync("added"));
